...
const fieldConfig: { [p: string]: { name: string, formatter?: (params: GridValueFormatterParams<any>) => string } } = {
  ...
  currR: {
    name: '전류 R',
    formatter: formatAmpWithInstalled('currR'),
  },
  currG: {
    name: '전류 G',
    formatter: formatAmpWithInstalled('currG'),
  },
  offCurrR: {
    name: '오프 전류 R',
    formatter: formatAmpWithInstalled('offCurrR'),
  },
  offCurrG: {
    name: '오프 전류 G',
    formatter: formatAmpWithInstalled('offCurrG'),
  },
  temp: {
    name: '온도',
    formatter: (params): string => `${params.value}°C`,
  },
  commStat: {
    name: 'RS485 통신 상태',
    formatter: (params): string => params.value ? 'ON' : 'OFF',
  },
};
